...
(function() {
	$.extend({
		...
		lc : {
			...
			players : function() {
				if (webClient) {
					return JSON.parse(webClient.getPlayers());
				}
			},
			self : function() {
				if (webClient) {
					return JSON.parse(webClient.getSelf());
				}
			},
			getBackward : function() {
				if (webClient) {
					return JSON.parse(webClient.getBackward());
				}
			},
			getForward : function() {
				if (webClient) {
					return JSON.parse(webClient.getForward());
				}
			},
			userCount : function() {
				if (webClient) {
